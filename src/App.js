...
import { 
  ...
  CircularProgress, Paper, Typography, Chip, Tooltip, Divider, Link, List, ListItem
} from '@mui/material';
import { 
    Send as SendIcon, 
...
} from '@mui/icons-material'; // Adicionei ícones novos
...
const ChatMessage = React.memo(function ChatMessage({ message }) {
  ...
  // Função auxiliar de ícones (similar ao ResultadoConsulta)
    ...
    <Box sx={{ display: 'flex', justifyContent: isUser ? 'flex-end' : 'flex-start', mb: 2 }}>
      <Paper 
        ...
        sx={{
          ...
          maxWidth: '85%', // Aumentei um pouco
          borderRadius: isUser ? '20px 20px 5px 20px' : '20px 20px 20px 5px',
          
          // Estilos Markdown... (Mantidos)
      ...
      >
        ...
        {/* --- SEÇÃO DE FONTES (RENDERIZADA APENAS SE HOUVER) --- */}
...
function App({ apiToken, userEmail, onLogout }) {
  ...
  const {
    ...
    setLastMessageSources // <-- Importando a nova ação
      ...
      switch (data.response_type) {
        case 'stream_answer':
          ...
          await fetchChatStream({
            ...
            // --- CONECTANDO O CALLBACK DE FONTES ---
